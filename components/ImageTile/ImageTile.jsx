...
const ImageTile = ({ image, alt, priority, borderRadius, handleClick }) => {
  const { image: url, base64 } = image
  const imageProps = useNextSanityImage(client, url)
  const  { src, loader } = imageProps
  
  return (
    <div className= {styles.container} style = {{borderRadius: borderRadius}} onClick = {() => handleClick(image.image)}>
      <Image 
        src = {src}
        loader = {loader}
        fill
